...
async def upsert_story_target(
    ...
    return await pool.fetchrow("""
        INSERT INTO story_targets (target_url, telegram_id, username, title)
        VALUES ($1, $2, $3, $4)
        ON CONFLICT (target_url) DO UPDATE
            SET telegram_id = COALESCE(EXCLUDED.telegram_id, story_targets.telegram_id),
                username    = COALESCE(EXCLUDED.username, story_targets.username),
                title       = COALESCE(EXCLUDED.title, story_targets.title),
                updated_at  = now()
        RETURNING *
