...
```bash
python3 -m app.main add-account
```

Старая команда `onboard` тоже работает:
...
4. API ID / API hash
5. Прокси: да/нет
6. QR-вход вместо кода: да/нет
7. Код подтверждения Telegram или сканирование QR
8. Пароль 2FA (если включен)
9. Настройки аккаунта (daily percent, max/day, sleep, delays)
10. Группы/чаты для работы
...
Файловый клиент создается в `app/auth.py` через `create_client(...)` и хранит временную сессию в `data/session_<имя>`.
...
- перед запуском слушателя аккаунт пытается вступить в целевые каналы и их linked-группы.
...
4. Попробуйте QR-вход в onboarding.
5. Если видите `PhoneNumberFloodError` или `SendCodeUnavailableError`, подождите 5-15 минут и повторите onboarding.
