...
Если код не пришел, введите `resend` (или `/resend`) для повторной отправки, либо `sms` (или `/sms`) для альтернативного повторного запроса. Telegram не гарантирует SMS и часто отправляет код в приложение.
...
4. На этапе ввода используйте:
   - `resend` для повторной отправки кода
   - `sms` для альтернативного повтора (Telegram не гарантирует SMS)
